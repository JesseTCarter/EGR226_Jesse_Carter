...
Description:    A program that checks if a button is pressed and held, if pressed a green LED turns
                on and if the button is continually held down, it will sequence through three
                different led colors, green, yellow, red. Incorporates a de-bounce switch
                function to solve for the switch bounce, so that the button presses are accurate
                with changing the LED colors, so that no interference accidentally changes which LED is lit.
